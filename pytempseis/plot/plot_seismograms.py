...
comp_list = ["Z", "R", "T"]
station_comp = []
filelist = glob.glob(f"{observed_folder}/*_P_observed.asc")
...
plt.subplots_adjust(
    bottom=0.05, left=0.05, right=0.98, top=0.95, hspace=0.5, wspace=0.25
)
...
for i in range(0, len(station_comp)):
    ...
    observed = f"{observed_folder}{station}_{comp}_{wavetype}_observed.asc"
    predicted = f"{predicted_folder}{station}_{comp}_{wavetype}_best_predicted.asc"
    point_source = f"{point_source_folder}{station}_{comp}_{wavetype}_ps"
    ...
    if c <= (
        len(station_comp) - b
    ):  # this is stupid as it will need changing for each different thing
        print(c)
        ax.set_xticklabels([])
...
plt.subplots_adjust(
    bottom=0.05, left=0.05, right=0.98, top=0.95, hspace=0.7, wspace=0.25
)
...
for i in range(0, len(station_comp)):
    try:
        ...
        if len(station) == 3:
            ...
            plt.annotate(
                str(mft), xy=(0.75, 0.14), xycoords="axes fraction", fontsize=11
            )
        if len(station) == 4:
            plt.annotate(
                station + " " + comp,
...
plt.savefig(results_folder + f"{results_folder}/seismograms_STEST2.pdf")
plt.close()
