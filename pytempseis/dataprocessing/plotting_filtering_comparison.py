...
for i in range(0, len(x)):
    ...
    os.system(f"mkdir ../database/{event_code}/filtering_testing")
...
for i in range(0, len(filelist)):
    ...
    fig = plt.figure(1, figsize=(11.69, 8.27))
    plt.suptitle(f"Station: {station[i]} Comp: {component[i]} Wavetype: {wavetype2[i]}")
    plt.subplot(411)
    plt.plot(
        np.arange(0, len(obs_inv_list[0][i]), 1),
        obs_inv_list[0][i].real,
        label="Observed",
        color="black",
        zorder=0,
        linewidth=3,
    ...
    plt.plot(
        np.arange(0, len(ps_inv_list[0][i]), 1),
        ps_inv_list[0][i].real,
        label="Point source",
        color="red",
        zorder=1,
    )
    plt.ylabel(f"Filter: {uplim[0]}s {lowlim[0]}s")
    plt.subplot(412)
    plt.plot(
        np.arange(0, len(obs_inv_list[1][i]), 1),
        obs_inv_list[1][i].real,
        label="Observed",
        color="black",
        zorder=0,
        linewidth=3,
    ...
    plt.plot(
        np.arange(0, len(ps_inv_list[1][i]), 1),
        ps_inv_list[1][i].real,
        label="Point source",
        color="red",
        zorder=1,
    )
    plt.ylabel(f"Filter: {uplim[1]}s {lowlim[1]}s")
    plt.subplot(413)
    plt.plot(
        np.arange(0, len(obs_inv_list[2][i]), 1),
        obs_inv_list[2][i].real,
        label="Observed",
        color="black",
        zorder=0,
        linewidth=3,
    ...
    plt.plot(
        np.arange(0, len(ps_inv_list[2][i]), 1),
        ps_inv_list[2][i].real,
        label="Point source",
        color="red",
        zorder=1,
    )
    plt.ylabel(f"Filter: {uplim[2]}s {lowlim[2]}s")
    plt.savefig(
        f"../database/{event_code}/filtering_testing/{station[i]}_{component[i]}_{wavetype2[i]}2.png"
    )
    plt.close()
