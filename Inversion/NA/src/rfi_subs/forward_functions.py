    ...
    f20 = np.matrix(
        [
            [
                ...
                float(lines[5].split()[2]),
                float(lines[5].split()[3]),
            ],
            [
            ...
            [
                ...
                float(lines[7].split()[2]),
                float(lines[7].split()[3]),
            ],
        ]
    )
...
def seismicmoment2par(f00, f10, f01, f02, f11, f20):
    ...
    W = np.matrix(
        [
            ...
            [fyx / M0, fyy / M0, fyz / M0],
            [fzx / M0, fzy / M0, fzz / M0],
        ]
    )
    ...
    l1 = np.sqrt(np.dot((r1.T * W), r1)[0, 0]) / 2.0
    l2 = np.sqrt(np.dot((r2.T * W), r2)[0, 0]) / 2.0
    l3 = np.sqrt(np.dot((r3.T * W), r3)[0, 0]) / 2.0
