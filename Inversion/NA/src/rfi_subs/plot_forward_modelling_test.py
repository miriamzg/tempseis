...
station = "CO_0"
observed_file = "../../data/rfi_files/OBS/" + station + "_Z_fft"
predicted_real_file = "./" + station + "_predicted_real.txt"
predicted_imag_file = "./" + station + "_predicted_imag.txt"
point_source_file = "point_source/" + station + "_Z_fft"
...
plt.subplot(511)
plt.plot(obs_freq[0:int(len(obs_freq)/2.)], obs_real[0:int(len(obs_freq)/2.)], color="red")
plt.plot(psource_freq[0:int(len(obs_freq)/2.)], psource_real[0:int(len(obs_freq)/2.)], color="black")
plt.xscale("log")
plt.xlim(0.01, 0.2)

plt.subplot(512)
plt.plot(obs_freq[0:int(len(obs_freq)/2.)-1], obs_real[0:int(len(obs_freq)/2.)-1], color="red")
plt.plot(pred_freq_real[0:int(len(obs_freq)/2.)-1], pred_real[0:int(len(obs_freq)/2.)-1], color="black")
plt.xscale("log")
plt.xlim(0.01, 0.2)
